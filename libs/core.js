...
function autocomplete_element(value, options, url) {
	  // creating input element
	var extra = '';
	if (options.size!=null) extra = ' size="' + options.size + '"';
	if (options.maxlength!=null) extra = extra + ' maxlength="' + options.maxlength + '"';
	
	var $ac = $('<input type="text"' + extra + '/>');
	// setting value to the one passed from jqGrid
	$ac.val(value);
	// creating autocomplete
	$ac.autocomplete({source: url});
